                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form action="{{route('admin.storeProduct_Charge')}}" method="POST"  enctype="multipart/form-data">
                                @csrf
                                <div class="mb-1">
        ...
        <br>
           <table id="example" class="table table-striped" style="width:100%">
            ...
            @foreach($products as $product)
                <tr>
                    <td>{{$loop->index+1}}</td>
                    <td><img src="{{asset('admin/admin_image/products/products_charge/'.$product->photo)}}" alt="no photo" style="width: 40px;height: 40px;border-radius:50%"></td>
                    <td>{{$product->name}}</td>
                    <td>{{$product->price}}</td>
                    ...
                    <td>
                        <a href="{{route('admin.deleteProduct_charge',$product->id)}}" class="control-delete btn btn-primary"  onclick="return confirm('هل انت متاكد من حذف المنتج')"><span class="fa-solid fa-trash"></span></a>
                    </td>
                </tr>
            @endforeach
            </tbody>
